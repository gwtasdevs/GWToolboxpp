...
namespace Pathing {
    ...
    GW::PathingTrapezoid* FindTrapezoid(const GW::GamePos& point)
    {
        GW::MapContext* mc = GW::GetMapContext();

        if (!mc || !mc->path || !mc->path->staticData || mc->path->staticData->map.size() <= point.zplane) return nullptr;

        auto& pm = mc->path->staticData->map[point.zplane];
    ...
    {
        GW::MapContext* mc = GW::GetMapContext();
        if (!mc || !mc->path || !mc->path->staticData || !mc->path->staticData->map.size()) return nullptr;

        GW::PathingTrapezoid* closest = FindTrapezoid(point);
        ...
        for (uint32_t i = 0; i < mc->path->staticData->map.size(); ++i) {
            auto& pm = mc->path->staticData->map[i];
    ...
    GW::PathingTrapezoid* FindClosestPositionOnTrapezoid(GW::GamePos& point)
    {
        ...
        GW::PathingTrapezoid* pt = FindClosestTrapezoid(point);
        if (pt) return pt;

        float closest_dist = std::numeric_limits<float>::infinity();
        GW::PathingTrapezoid* closest = nullptr;

        for (uint32_t z = 0; z < mc->path->staticData->map.size(); ++z) {
            const auto& m = mc->path->staticData->map[z];
    ...
    public:
        ...
        Portal(Portal::id id,
            ...
            PointId start_id,
            PointId goal_id,
            const GW::PathingTrapezoid *pt, uint8_t pt_layer, 
            Edge edge);
	...
	Portal::Portal(Portal::id id, Portal::id other_id,
        PointId start_id,
        PointId goal_id,
        const GW::PathingTrapezoid* pt, uint8_t pt_layer,
        Edge edge) :
    ...
    public:
        ...
        Point(const GW::Vec2f &pos, PointId id, Portal::id portal1_id, Portal::id portal2_id, bool is_viable) :
            m_id(id), m_pos(pos), m_portals{ portal1_id, portal2_id }, is_viable(is_viable) {
        };
        GW::Vec2f m_pos;
	...
	struct VisitedState {
		...
		inline size_t checkpoint() const {
			return stack.size();
		}
	...
	void createPortalPair(const GW::PathingTrapezoid* pt1, uint8_t pt1_layer, const GW::PathingTrapezoid* pt2, uint8_t pt2_layer,
        ...
        GW::Vec2f p2, bool p2_viability, Edge edge) {

        auto id = static_cast<Portal::id>(portals.size());
        auto p_id = static_cast<PointId>(points.size());
		
        points.emplace_back(p1, static_cast<PointId>(points.size()), 
			id, id + 1, p1_viability);
					
        points.emplace_back(p2, static_cast<PointId>(points.size()), 
			id, id + 1, p2_viability);
			
        portals.emplace_back(id, id + 1,
            p_id, p_id + 1,
            pt1, pt1_layer,
            edge);

        portals.emplace_back(id + 1, id,
            p_id + 1, p_id,
            pt2, pt2_layer,
            (Edge)(((uint32_t)edge + 2u) % 4u));
	...
	void GetTrapezoidNeighbours(GW::PathingMap& m, const GW::PathingTrapezoid* trapezoid, uint8_t trapezoid_layer, std::vector<Neighbour>& neighbours) {
                ...
                if (portal && portal->pair) {
                    ...
                    for (auto it = begin; it < end; ++it) {
                        ...
                        if ((*it)->YB == (*it)->YT) { // Trapezoid is degenerate
                            if (between(trapezoid->XTL, (*it)->XBL, (*it)->XBR) ||
                                between(trapezoid->XTR, (*it)->XBL, (*it)->XBR) ||
                            ...
                            if (onSegment({ trapezoid->XTL, trapezoid->YT }, { (*it)->XTR, (*it)->YT }, { trapezoid->XBL, trapezoid->YB }) ||
                                onSegment({ trapezoid->XTL, trapezoid->YT }, { (*it)->XBR, (*it)->YB }, { trapezoid->XBL, trapezoid->YB }) ||
                                onSegment({ (*it)->XTR, (*it)->YT }, { trapezoid->XTL, trapezoid->YT }, { (*it)->XBR, (*it)->YB }) ||
                                onSegment({ (*it)->XTR, (*it)->YT }, { trapezoid->XTL, trapezoid->YT }, { (*it)->XBR, (*it)->YB })) {
            ...
            if (n.t->portal_right < m.portal_count) {
                ...
                if (portal && portal->pair) {
                    ...
                    for (auto it = begin; it < end; ++it) {
                        ...
                        if (visited[(*it)]) continue;
                        if ((*it)->YT == (*it)->YB) {
                            if (between(trapezoid->XBL, (*it)->XTL, (*it)->XTR) ||
                            ...
                            if (onSegment({ trapezoid->XTR, trapezoid->YT }, { (*it)->XTL, (*it)->YT }, { trapezoid->XBR, trapezoid->YB }) ||
                                onSegment({ trapezoid->XTR, trapezoid->YT }, { (*it)->XBL, (*it)->YB }, { trapezoid->XBR, trapezoid->YB }) ||
                                onSegment({ (*it)->XTL, (*it)->YT }, { trapezoid->XTR, trapezoid->YT }, { (*it)->XBL, (*it)->YB }) ||
                                onSegment({ (*it)->XTL, (*it)->YT }, { trapezoid->XTR, trapezoid->YT }, { (*it)->XBL, (*it)->YB })) {
    ...
    bool isNeighbourOf(const GW::PathingTrapezoid* pt1, uint8_t pt1_layer, const GW::PathingTrapezoid* pt2) {
        ...
        auto& m = (*map)[pt1_layer];
        GetTrapezoidNeighbours(m, pt1, pt1_layer, neighbours);
        return std::ranges::any_of(neighbours, [&](const auto& n) { return n.t == pt2; });
    }
	...
	void linkTrapezoids(const GW::PathingTrapezoid* pt1, uint8_t pt1_layer, const Neighbour& n) {
        ...
        if (edge == Edge::top) {
            ...
            createPortalPair(pt1, pt1_layer, pt2, pt2_layer,
                a, point1_viability,
                b, point2_viability, edge);
        }
    ...
    void GenerateTrapezoidNeighbours() {
        ...
        GW::PathingMapArray* map = GW::Map::GetPathingMap();
        GW::MapContext* mapContex = GW::GetMapContext();
    ...
    void GeneratePortals() {
        ...
        auto reserved_points = static_cast<int>(mapContex->path->pathNodes.size() * 2.2) + m_teleports.size() * 2;
        points.clear();
        points.reserve(reserved_points);
        portals.clear();
        portals.reserve(reserved_points);
        pt_portal_map.clear();
        pt_portal_map.resize(mapContex->path->pathNodes.size());

        portal_pt_map.clear();

        int pitch = mapContex->path->pathNodes.size();
    ...
    inline void process_portal(
        std::vector<node>& open,
        size_t& sp,
        VisitedState& visited,
        const Portal& portal,
        Portal::id other_portal_id
    ) {
        ...
        for (auto pid : portal_portal_map[portal.m_id]) {
            if (pid == portal.m_id || pid == other_portal_id)
                continue;
            ...
            if (portal.m_pt_layer)
                n.blocked_planes.set(portal.m_pt_layer, true);
    ...
    Point& createSinglePointPortal(const GW::PathingTrapezoid* pt, const GW::GamePos& gp) {
        ...
        portals.emplace_back(id, id,
            point_id, point_id,
            pt, static_cast<uint8_t>(gp.zplane),
            Edge::top);
    ...
    void VisGraphInsertPoint(std::vector<node>& open, const Point& point) {
        ...
        while (sp && --cnt) {
            ...
            if (visited.is_visited(portal.m_id))
                continue;

            visited.visit(portal.m_id);

            if (portal.m_pt_layer)
                cur.blocked_planes.set(portal.m_pt_layer, true);
            ...
            if (fr_nl < -tolerance || fl_nr > tolerance)
                continue;
...
namespace Pathing {
	...
	void MilePath::stopProcessing() { mImpl->m_terminateThread = true; }
    ...
    Error BuildPath(AStar &astar, const Point& start, const Point& goal, const std::vector<PointId>& came_from)
    {
        ...
        while (current.m_id != start.m_id) {
            ...
            auto zplane = std::max(mp->portals[current.m_portals[0]].m_pt_layer, 
									mp->portals[current.m_portals[1]].m_pt_layer);
		...
		auto zplane = std::max(mp->portals[start.m_portals[0]].m_pt_layer, 
								mp->portals[start.m_portals[1]].m_pt_layer);
        astar.m_path.insertPoint({start.m_pos.x, start.m_pos.y, zplane});

        return Error::OK;
    }

    float TeleporterHeuristic(const MapSpecific::Teleports &teleports, 
								const std::vector<MapSpecific::teleport_node> &teleportGraph, 
								const Point& start, const Point& goal)
    {
        ...
        for (const auto& tp : teleports) {
            ...
            if (tp.m_directionality == Teleport::direction::both_ways)
                dist = std::min(dist, GW::GetSquareDistance(start.m_pos, tp.m_exit));
            ...
            if (tp.m_directionality == Teleport::direction::both_ways)
                dist = std::min(dist, GW::GetSquareDistance(goal.m_pos, tp.m_enter));
