...
namespace {
    ...
    std::filesystem::path GetComputerFolderPath()
    {
        ...
        PathCreateDirectorySafe(docpath);

        return docpath;
    }

    std::filesystem::path GetPath(const std::filesystem::path& file)
    {
    ...
    std::vector<FontData>& GetFontData()
    {
        ...
        for (const auto& [font_name, glyph_range] : fonts_on_disk) {
            const auto path = GetPath(font_name);
    ...
    ImFont* BuildFont(const float size, const bool default_only = false)
    {
        ...
        if (default_only) {
            ...
            for (const auto& [glyph_ranges, font_name] : GetFontData()) {
                ...
                const auto font_name_str = PluginUtils::WStringToString(font_name);
